        ...
        <div
         ...
         @if ($role === 'admin')
            ...
            @foreach (range(1, 6) as $i)
                @includeIf('components.sidebar.sidebar-area' . $i)
            @endforeach

            @include('components.sidebar.hasil')
            @include('components.sidebar.validasi')

        {{-- Sidebar untuk Manager Area --}}
        @elseif (str_starts_with($role, 'area'))
            @php
                $areaNumber = (int) filter_var($role, FILTER_SANITIZE_NUMBER_INT);
            @endphp
            @includeIf('components.sidebar.sidebar-area' . $areaNumber)
        @endif
</nav>
</aside>